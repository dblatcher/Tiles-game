...
const gameActions = {
        ...
        units.forEach(unit => {
            if (unit.faction === state.activeFaction) {
                ...
                if (unit.onGoingOrder) {
                    unit.onGoingOrder.timeRemaining -= unit.type[unit.onGoingOrder.type.requiredUnitSkill]
                }
    ...
    handleUnitClick: clickedUnit => state => {
        ...
        if (state.interfaceMode === 'MOVE') {
            if (clickedAndSelectedUnitAreOnSameSquare) {
                ...
                state.pendingBattle = new Battle (state.selectedUnit, unitsOnSameSquareAsClickedUnit, state.mapGrid[clickedUnit.y][clickedUnit.x])
            }
    ...
    holdUnit: state => {
        state.selectedUnit.remainingMoves = 0;
        selectNextOrPreviousUnit(state)
        return state
    },

    killUnit: input => state => {

        const casualty = input.unit
        if (!casualty && !state.units.includes(casualty)) {
            console.log('They\'re dead... all of them')
            state.selectedUnit = null
        } else {

            console.log('I must kill...', casualty.infoList[0])
            ...
            if (state.selectedUnit === casualty) { selectNextOrPreviousUnit(state) }
