...
const gameActions = {
    ...
    handleMapSquareClick: mapSquare => state => {
        ...
        const enemyUnitsInClickedSqaure = unitsInClickedSquare
        ...
        if (enemyUnitsInClickedSqaure.length === 0) {
            attemptMove(state, state.selectedUnit, mapSquare)
            return state;
        } else {

            if (state.selectedUnit.type.attack > 0) {
                state.pendingBattle = new Battle(state.selectedUnit, enemyUnitsInClickedSqaure, mapSquare)
            } else {
