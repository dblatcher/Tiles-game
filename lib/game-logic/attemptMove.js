...
const attemptMove = (state, unit, mapSquare) => {
    ...
    if (townInMapSquare && townInMapSquare.faction !== unit.faction) {
        if (!unit.canMoveToOrAttack(mapSquare, unitSquare, townInMapSquare, unitsInMapSquare)) {
            if (!state.activeFaction.isComputerPlayer) {
    ...
    if (!unit.type.isNaval && mapSquare.isWater) {
        if (!unit.canMoveToOrAttack(mapSquare, unitSquare, townInMapSquare, unitsInMapSquare)) { return false }
        ...
        console.log('BOARDING SHIP?', transports)
