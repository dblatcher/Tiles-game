...
TradeBudget.prototype.allocate = function (totalTrade) {
    ...
    let shortfalls={}, biggestShortfall,valueWithBiggestShortFall;

    while (remainder > 0) {
        biggestShortfall = 0
        valueNames.forEach(valueName => {
            shortfalls[valueName] = (this.store[valueName] * totalTrade) - output[valueName]
            if (shortfalls[valueName] > biggestShortfall) {
                biggestShortfall = shortfalls[valueName]
                valueWithBiggestShortFall = valueName
            }
        })
        output[valueWithBiggestShortFall]++
        remainder--
    }
