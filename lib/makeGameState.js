...
const makeGameState = {
    ...
    test() {
        ...
        const mapGrid = MapSquare.makeGrid(30, 18, function (x, y) {
            const inWater = (
                !(x - y * 1.5 < 3)
                || y + 2.2 * x > 26
                || (y == 10 && x == 2)
        ...
        const units = [
            ...
            new Unit(unitTypes.knight, factions[0], { x: 6, y: 3 }),
        ...
        const towns = [
            new Town(factions[0], mapGrid[1][2], { name: "Jamestown", population:4 }),
            new Town(factions[0], mapGrid[16][29], { name: "Plymouth" }),
            new Town(factions[1], mapGrid[8][6], { name: "Montreal", population:2 }),
